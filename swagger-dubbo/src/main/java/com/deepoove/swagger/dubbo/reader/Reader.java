...
public class Reader {
	...
	private void read(ReaderContext context) {
		...
		Class<?> interfaceCls = context.getInterfaceCls();
		Method[] methods = interfaceCls.getMethods();
		List<Method> interfaceMethodList = new ArrayList<>();
		Collections.addAll(interfaceMethodList, methods);
//		List<Method> interfaceMethodList = Arrays.asList(declaredMethods);//转换之后，将方法返回对象的泛型参数丢了
